...
export default async function uploadImage(image) {
    ...
    const {error: uploadError} = await supabase.storage.from('car_photos').upload(imagePath, image);
    if (uploadError) {
        console.log("Uploading Error: ", uploadError.message);
        return {data: "", error: uploadError}
    }

    const {data, error: getUrlError} = await supabase.storage.from('car_photos').getPublicUrl(imagePath);
    if (getUrlError) {
